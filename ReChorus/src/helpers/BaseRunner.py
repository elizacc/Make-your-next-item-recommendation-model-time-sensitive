...
class BaseRunner(object):
    ...
    def parse_runner_args(parser):
        ...
        parser.add_argument('--metric', type=str, default='NDCG,HR,MAP',
                            help='metrics: NDCG, HR, MAP')
        return parser
        ...
        for k in topk:
            ...
            for metric in metrics:
                ...
                if metric == 'HR':
                    ...
                    evaluations[key] = np.mean(np.sum([hit / i * (gt_rank == i) for i in range(1, k+1)], axis=0))
                # elif metric == 'MAP(norm)':
                #     ap = np.sum([hit / i for i in range(1, k+1)], axis=0)
                #     evaluations[key] = np.mean(ap / ap.max())
                elif metric == 'MRR':
                    evaluations[key] = np.mean((1 / gt_rank) * hit)
                else:
