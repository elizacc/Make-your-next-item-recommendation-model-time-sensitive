...
class KDA(SequentialModel):
    ...
    def __init__(self, args, corpus):
        ...
        if self.prediction_time == 'hour':
            self.num_uniq_values = 24
        elif self.prediction_time == 'day':
            self.num_uniq_values = 7
        elif self.prediction_time == 'month':
            self.num_uniq_values = 12
    ...
    def _define_params(self):
        ...
        if self.prediction_time != 'no':
            self.time_embeddings = nn.Embedding(self.num_uniq_values + 1, self.emb_size)
    ...
    def rec_forward(self, feed_dict):
        ...
        if self.prediction_time == 'no':
            ...
            if self.version == 2:
                his_vectors = torch.cat((self.entity_embeddings(history), self.time_embeddings(history_times_shift)), dim=2)  # B * H * 2V
                h_linear = nn.Linear(2 * self.emb_size, self.emb_size, device=self.device)
            elif self.version == 3:
                his_vectors = torch.cat((self.entity_embeddings(history), self.time_embeddings(history_times), self.time_embeddings(history_times_shift)), dim=2) # B * H * 3V
                h_linear = nn.Linear(3 * self.emb_size, self.emb_size, device=self.device)
            
            his_vectors = h_linear(his_vectors)  # B * H * V
